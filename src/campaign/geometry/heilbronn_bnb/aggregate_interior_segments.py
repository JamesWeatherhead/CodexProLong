#!/usr/bin/env python3
"""Audit and merge disjoint fully-interior first-pair DFS intervals."""
...
def main() -> int:
    ...
    for path in args.summaries:
        ...
        if summary.get("side_counts") != [0, 0, 0]:
            raise ValueError(f"{path}: not a fully interior search")
        if record["status"] == "feasible":
            raise ValueError(f"{path}: segment contains a feasible configuration")
        start = int(record["start_root_pair"])
        completed = int(record["completed_root_pairs"])
        ...
        total_roots_set.add(int(record["total_root_pairs"]))
    ...
    output = {
        "mode": "audited union of exact fully-interior first-pair intervals",
        "side_counts": [0, 0, 0],
