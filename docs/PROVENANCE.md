...
- Campaign model: GPT-5.6 Sol, used through Codex.
...
winning submission events were recorded under one campaign turn using
GPT-5.6 Sol through Codex at `ultra` reasoning effort, and the candidate hashes
in those events match the five retained receipts.

This is local provenance, not an OpenAI-signed attestation. Public claims are
therefore anchored to Arena solution IDs, candidate hashes, verifier hashes,
scores, and timestamps rather than to a claim of model reproducibility.
